...
export class StarshipClient {
  ...
  async initClient() {
    ...
    if (chainId !== this.chainInfo['chain_id']) {
      throw Error(
        `Given ChainId: ${this.getChainId()} doesn't match the clients ChainID: ${chainId}!`
      );
    }
  ...
  get rpc() {
    const chain = this.config['chains'].find(
      (chain) => chain.name === this.chainId
    );
  ...
  getGasPrice() {
    const price =
      this.chainInfo['fees']['fee_tokens'][0]['fixed_min_gas_price'];
  ...
  getRegistryUrl() {
    const port = this.config.hasOwnProperty('registry')
  ...
  async getIBCInfo(chain2) {
    const resp = await fetch(
      `${this.getRegistryUrl()}/ibc/${this.chainId}/${chain2}`,
      {}
    );
  ...
  async fetchChainInfo() {
    const url =
      'http://localhost:' +
      (this.config.hasOwnProperty('registry')
        ? this.config.registry.ports.rest
        : 8080);
  ...
  async sendIBCTokens(toAddress, amount, chain2) {
    ...
    const resp = await this.client.sendIbcTokens(
      ...
      channel['channel_id'],
      {
        revisionHeight: Long.fromNumber(12300),
        revisionNumber: Long.fromNumber(45600)
      },
  ...
  static async withConfigFile(configFile, chainOpts = {}) {
    ...
    await Promise.all(
      chainIds.map(async (chainId) => {
        const chainOpt = chainOpts.hasOwnProperty(chainId)
          ? chainOpts[chainId]
          : {};
        chainClients[chainId] = await StarshipClient.withChainId(
          chainId,
          config,
          chainOpt
        );
